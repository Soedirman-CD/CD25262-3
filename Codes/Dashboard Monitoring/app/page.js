...
export default function Dashboard() {
        ...
        try {
          ...
          setHistory(prev => {
            const updated = [...prev, {
              ...
              tegangan: newRow.tegangan != null ? +Number(newRow.tegangan).toFixed(1) : 0,
              arus:     newRow.arus     != null ? +Number(newRow.arus).toFixed(2)     : 0,
            }]
  ...
  // Hitung estimasi tagihan langsung dari energi PZEM (akumulatif)
  const energiKwh   = data?.energi ?? 0
  const tagihan     = Math.round(energiKwh * TARIF_PER_KWH)
  const biayaStr    = tagihan.toLocaleString('id-ID')
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-4 md:p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        ...
        />
        <GaugeCard
          label="Arus" value={round(data?.arus, 2)}
          unit="A" min={0} max={20} color="green"
        />
        <GaugeCard
          label="Daya" value={round(data?.daya, 0)}
          unit="W" min={0} max={4400} color="yellow"
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        ...
        <StatCard
          label="Power Factor"
          value={round(data?.pf, 2)}
          unit=""
          colorClass="text-green-400"
          icon="📊"
          sub={`Frekuensi: ${round(data?.frekuensi, 1) ?? '—'} Hz`}
        />
      </div>
        ...
        {history.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={160}>
            <LineChart data={history}>
              ...
              <XAxis dataKey="time" tick={{ fill: '#4b5563', fontSize: 9 }}
                interval="preserveStartEnd" />
              <YAxis tick={{ fill: '#4b5563', fontSize: 10 }} unit=" A" />
              <Tooltip content={<CustomTooltip />} />
              <Line type="monotone" dataKey="arus" stroke="#4ade80"
                strokeWidth={2} dot={false} name="Arus" unit="A" />
            </LineChart>
          </ResponsiveContainer>
        )}
      </div>
